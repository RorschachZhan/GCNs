...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, motifs_all, motifs_num):
        for i in range(self.k):
            temp = [(torch.mm(motifs_all[mat], x).T / motifs_num[mat]).T for mat in range(len(motifs_all))]
            temp2 = torch.stack(temp, 0)
            x_agg = torch.mean(temp2, 0)
            x = self.activation(self.conv[i](x_agg, edge_index))
        return x
...
class Model(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor,
                edge_index: torch.Tensor, motifs_all, motifs_num) -> torch.Tensor:
        ...
        return self.encoder(x, edge_index, motifs_all, motifs_num)
