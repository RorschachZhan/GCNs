...
def tst(model: Model, x, edge_index, y, motifs, motifs_num, final=False):
    model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='/home/dell3/PengY/MOTIF_EDGE/grace/config.yaml')
    parser.add_argument('--withone', type=int, default=0)#0表示不要1阶motif
    args = parser.parse_args()
    assert args.gpu_id in range(0, 8)
    torch.cuda.set_device(args.gpu_id)
    for datasets in ["acmv9"]:
        args.withone = 1
        allx, ally, graph, motifs_all, motifs_num = loadAllData(datasets, args.withone)
        lr = 0.001
        tau = 0.4
        config = yaml.load(open(args.config), Loader=SafeLoader)['Football']
        ...
        random.seed(12345)
        loss_path = "./res/" + datasets + "_loss.txt"
        ...
        for _ in range(1):#重复执行20次
            ...
            best = 1e9
            best_t = 0
            with open("/home/dell3/PengY/MOTIF_EDGE/pascal/mean_concat/" + datasets + ".txt", 'a+') as res_file:
                # res_file.write(str(config) + "\n")
                for epoch in range(1, 1501):
                    loss = train(model, allx, edges_index, motifs_all, motifs_num)
                    loss_file.write('epoch: {0}, Loss: {1:0.4f}\n'.format(epoch, loss))
                    print('Epoch: {0}, Loss: {1:0.4f}'.format(epoch, loss))
                #     if loss < best:
                #         best = loss
                #         best_t = epoch
                #         cnt_wait = 0
                #         torch.save(model.state_dict(), '/home/dell3/PengY/MOTIF_EDGE/pascal/mean_concat/model.pkl')
                #     else:
                #         cnt_wait += 1
                ...
                # model.load_state_dict(torch.load('/home/dell3/PengY/MOTIF_EDGE/pascal/mean_concat/model.pkl'))
                # res = tst(model, allx, edges_index, ally, motifs_all, motifs_num, final=True)
