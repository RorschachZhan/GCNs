...
DIR_PATH = "/home/dell3/PengY/GRACE-master/datasets"
WORK_PATH = "/home/dell3/PengY/GRACE-master/datasets/"
...
def buildfeaturenetwork(path, name, train_num, test_num):
    ...
    allx = feature[0: node_num]
    tx = feature[node_num - test_num: node_num]
    y = label[0: train_num]
    ally = label[0: node_num]
        ...
        while q.empty() == False:
            ...
            while check_flag == False:
                if time.time() - temp_time > 15:
                    ...
                    print(
                        "    Difficulty in processing node [{:5d}], reducing the cosine similarity threshold to [{:.2f}] in this process.".format(
                            node, local_cos_limited))
                    temp_time = time.time()
                    local_cos_reduce += 1
...
def loadAllData(dataset_str, withone):
    # names = ['allx.shuffle', 'ally.shuffle', 'graph.shuffle', 'motifs.all.shuffle', 'motifs.num.shuffle']
    names = ['allx', 'ally', 'graph', 'motifs.all.withone', 'motifs.num.withone']
    # names2 = ['allx.shuffle', 'ally.shuffle', 'graph.shuffle', 'motifs.all.withone.shuffle', 'motifs.num.withone']
    # names3 = ['allx.shuffle', 'ally.shuffle', 'graph.shuffle', 'motifs.all.degree', 'motifs.num.degree']
    # if withone == 0:
    #     names = names1
    # elif withone == 1:
    #     names = names2
    ...
    for i in range(len(names)):
        with open("/home/dell3/PengY/GRACE-master/motif_gcn/data/" + "ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
...
if __name__ == "__main__":
    ...
    # path = "D:\\图学习代码\\GRACE-master\\datasets\\TerrorAttack\\"
    # buildfeaturenetwork(path, network_name, train_num = 120, test_num = 500)
    # network_name = "email"
    # path = "D:\\图学习代码\\GRACE-master\\datasets\\email\\"
    # changelabelnetwork(path, "email-Eucore", train_num = 120, test_num = 500)

    # network_name = "facebook"
    # path = "D:\\图学习代码\\GRACE-master\\datasets\\Facebook\\"
    # changelabelnetwork(path, "fb_CMU_Carnegie49", train_num=120, test_num=500)

    # network_name = "polblogs"
    # path = "D:\\图学习代码\\GRACE-master\\datasets\\Polblogs\\"
    # changelabelnetwork(path, "polblogs", train_num=120, test_num=500)

    network_name = "football"
    path = "D:\\图学习代码\\GRACE-master\\datasets\\Football\\"
    changelabelnetwork(path, "football", train_num=120, test_num=500)