...
def motiffeature(g, Sparse = False):
    ...
    for node_a in node_list:
        ...
        for node_b in Na:
            ...
            for node_c in inse:
                ...
                motif_dict[node_a][1].add(tuple(sorted((node_a, node_a,node_b,node_b,node_c,node_c))))
                motif_dict[node_b][1].add(tuple(sorted((node_a, node_a,node_b,node_b,node_c,node_c))))
                motif_dict[node_c][1].add(tuple(sorted((node_a, node_a,node_b,node_b,node_c,node_c))))
            ...
            for i in range(0, len(VFlag)):
                if VFlag[i] == 1:
                    ...
                    nm_dict[i][1]      += 1/2
                    motif_dict[node_a][2].add(tuple(sorted((node_a, node_a, node_b, i))))
                    motif_dict[node_b][2].add(tuple(sorted((node_a, node_a, node_b, i))))
                    motif_dict[i][2].add(tuple(sorted((node_a, node_a, node_b, i))))
                elif VFlag[i] == 2:
                    # b在中间
                    nm_dict[node_a][1] += 1 / 2
                    nm_dict[node_b][1] += 1 / 2
                    nm_dict[i][1] += 1 / 2
                    motif_dict[node_a][2].add(tuple(sorted((node_a, node_b, node_b, i))))
                    motif_dict[node_b][2].add(tuple(sorted((node_a, node_b, node_b, i))))
                    motif_dict[i][2].add(tuple(sorted((node_a, node_b, node_b, i))))
            ...
            for node_c in inse:
                ...
                for node_d in Nc:
                    if VFlag[node_d] == 3:
                        ...
                        temp = (node_a, node_b, node_c, node_d)
                        motif_dict[node_a][3].add(tuple(sorted((node_a, node_b, node_c, node_d)*3)))
                        motif_dict[node_b][3].add(tuple(sorted((node_a, node_b, node_c, node_d)*3)))
                        motif_dict[node_c][3].add(tuple(sorted((node_a, node_b, node_c, node_d)*3)))
                        motif_dict[node_d][3].add(tuple(sorted((node_a, node_b, node_c, node_d)*3)))
                    elif VFlag[node_d] == 2:
                        # M42
                        ...
                        temp = (node_a, node_a, node_b, node_b, node_b, node_c, node_c, node_c, node_d, node_d)
                        motif_dict[node_a][4].add(tuple(sorted(temp)))
                        motif_dict[node_b][4].add(tuple(sorted(temp)))
                        motif_dict[node_c][4].add(tuple(sorted(temp)))
                        motif_dict[node_d][4].add(tuple(sorted(temp)))
                    elif VFlag[node_d] == 1:
                        temp = (node_a, node_a, node_a, node_b, node_b, node_c, node_c, node_c, node_d, node_d)
                        motif_dict[node_a][4].add(tuple(sorted(temp)))
                        motif_dict[node_b][4].add(tuple(sorted(temp)))
                        motif_dict[node_c][4].add(tuple(sorted(temp)))
                        motif_dict[node_d][4].add(tuple(sorted(temp)))
            ...
            for node_c in Na:
                ...
                for node_d in Nc:
                    if VFlag[node_d] == 2 and node_d != node_a:
                        ...
                        motif_dict[node_a][5].add(tuple(sorted((node_a, node_b, node_c, node_d))))
                        motif_dict[node_b][5].add(tuple(sorted((node_a, node_b, node_c, node_d))))
                        motif_dict[node_c][5].add(tuple(sorted((node_a, node_b, node_c, node_d))))
                        motif_dict[node_d][5].add(tuple(sorted((node_a, node_b, node_c, node_d))))
    ...
    return motif_feature, motif_dict
...
def totalMotifCounting(dataset_str, vis = False, slice=False):
    ...
    motif_feature, motif_dict = motiffeature(g, False)
    with open(PATH + "ind.{}.motif.allx".format(dataset_str), "wb") as f:
        pkl.dump(matrix2coo(motif_feature), f)
    with open(PATH + "ind.{}.motif.dict".format(dataset_str), "wb") as f:
        pkl.dump(motif_dict, f)
    if vis == True:
        line = []
...
if __name__ == "__main__":
    totalMotifCounting("amazon-computers")
